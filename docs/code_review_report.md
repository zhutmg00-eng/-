...
| # | 严重程度 | 问题描述 |
|---|---------|---------|
| 16 | 🔴严重 | `QUOTA_BENCHMARK`中的配额基准值（72.0、42.0等）为估算值，`docstring`注释"参照碳市场历史配额分配数据估算"，但未注明具体参考来源。这些值直接影响企业的配额缺口计算结果，需注明出处或标注"仅用于原型验证" |
...
| 44 | 🟢建议 | "生成报告"页面的PDF导出功能未实现（`st.info("PDF导出功能开发中")`），虽然README中未承诺此功能，但UI中展示未完成功能影响用户体验 |
...
| `src/engine/quota.py` | 70 | 配额缺口估算 |
| `src/engine/carbon_price.py` | 95 | 碳价参考与合规成本估算 |
